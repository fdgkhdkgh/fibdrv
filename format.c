...
// workaround
// take from https://github.com/BoschSensortec/BMG250-API/blob/master/bmg250_defs.h
...
void apm_to_string(const apm_digit *u, apm_size size, unsigned int radix, char *str, int inbytes) {
    ...
    // 我猜：
    // 原本為了要支援所有 base 的輸出，所以會用到浮點數運算
    // 我在這邊偷懶一下...
    //const size_t string_size = apm_string_size(size, radix) + 1;	
    //const size_t string_size = size * 10 + 1;
    char *tmpstr = MALLOC(512);
    char *p = apm_get_str(u, size, radix, str);
    /*int i;
    for(i = size - 1;i >= 0;i--) {
        snprintf(tmpstr, 512, "%llu", u[i]);
        //snprintf(tmpstr, 512, "%x", u[i]);
        strncat(str, tmpstr, inbytes);
    }*/
    
    //strncat(str, p, inbytes);
    FREE(tmpstr);
}

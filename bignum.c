...
// 我猜： 
//   size  是表示目前數字需要用到多少 element
//   alloc 是表示實際上，我們 alloc 多少個 element 了 
#define BN_SIZE(n, s)                                                          \
    do {                                                                       \
        bn *const __n = (n);                                                   \
        __n->size = (s);                                                       \
...
void bn_to_string(const bn *n, char *str, int inbytes, unsigned int base)
{
    ...
    memset(str, 0, sizeof(char) * inbytes);
    ...
    apm_to_string(n->digits, n->size, base, str, inbytes);
    // digits : 資料
    // size   : 用了多少個 element 
    // base   : hex or decimal
    //amp_to_string(n->digits, n->size, base);
}

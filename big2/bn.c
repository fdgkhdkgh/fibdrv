...
// 目前只支援 16 進位的字串
// TODO : 列印出 10 進位的字串
// Q: 要怎麼“有效率地”同時支援兩種列印方式？
// 我目前唯一能想到的就是改變大數的儲存方式，使其容易以十進位的形式輸出。 ex :
// 現在一個 element 存了 4bytes 的數字，可以改成每個 element 只能存最多 100000
// ， 這樣就能很簡單的用十進位的形式輸出
void bignum_to_string(struct bn *n, char *str, int nbytes)
{
    ...
    while ((j >= 0) && (nbytes > (i + 1))) {
        snprintf(&str[i], 9, SPRINTF_FORMAT_STR, n->array[j]);
